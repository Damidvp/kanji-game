...
## 4quater. Deuxième round de retours (2026-07-13, même fichier)

- **Bug réel, corrigé** : "Lancer la partie" était cliquable même si tous les joueurs n'étaient pas prêts. `GameRoomService.startGame` renvoie maintenant 409 si un participant actif n'est pas `ready` ; `LobbyScreen.tsx` désactive le bouton et affiche un message tant que ce n'est pas le cas.
- **Bug réel, corrigé** : au tout premier chargement d'un salon via lien partagé, `POST /join` échouait parfois une fois puis réussissait au clic sur "Réessayer". Cause : React StrictMode (dev) monte l'effet de connexion deux fois de suite au premier montage, envoyant deux vraies requêtes `/join` concurrentes pour le même `sessionToken`. `useRoomConnection` utilise maintenant un `AbortController` (annule réellement la requête du montage fantôme) au lieu d'un simple flag ignorant la réponse.
- **Bug réel, corrigé — celui-ci a pris du temps à cerner** : le dessin ne fonctionnait jamais sur la 1ère question d'Écriture (bloqué sur "Chargement des données de tracé..."), mais marchait normalement à partir de la 2e. Cause réelle (rien à voir avec StrictMode, contrairement à une première hypothèse) : `WritingScreen` affiche un écran "Connexion à la manche…" tant que `roomState` n'a pas fini de charger (le `join` REST est async), donc la `<div ref={targetRef}>` où `HanziWriter.create()` doit se monter n'existe pas encore dans le DOM à ce moment-là. L'effet qui crée le writer HanziWriter ne dépendait que de `round.roundIndex` (déjà connu immédiatement via l'état de navigation) : son tout premier passage trouvait la ref à `null`, ne faisait rien, et ne se redéclenchait jamais puisque `roundIndex` ne change pas tant qu'on reste sur la même manche — d'où "seule la 1ère question" est cassée (les suivantes tombent après que `roomState` soit chargé depuis longtemps). Fix : ajout de `hasRoomState` (`roomState !== null`, un booléen qui ne bascule qu'une fois) aux dépendances de cet effet.
  - Au passage, une **vraie erreur non interceptée a été trouvée et corrigée** : `client.publish()` de `@stomp/stompjs` lève une exception synchrone si appelée avant la fin du handshake WebSocket. `useRoomSocket`'s `sendAnswer`/`sendEnterLobby` vérifient maintenant `client?.connected` avant de publier (no-op silencieux sinon) plutôt que de planter.
  - **Méthodologique** : cette session a aussi révélé que le serveur de dev Vite peut rester bloqué à servir du code JS obsolète après de très nombreux hot-reloads successifs (HMR corrompu), sans erreur visible — un `console.log` ajouté ne s'affichait jamais alors que le fichier servi par `curl` le contenait bien. Un redémarrage complet du serveur (`preview_stop`/`preview_start`, pas juste un rechargement de page) a été nécessaire pour obtenir un signal fiable. À garder en tête si un futur débogage semble "ne rien changer" malgré des éditions de code correctes.
